package cn.chengzhiya.mhdfoptimize.util.network;
...
public final class HttpUtil {
     ...
     * @return 文件数据
     */
    public static byte[] downloadFile(URLConnection connection) {
        try {
            try (InputStream in = connection.getInputStream()) {
                byte[] bytes = ByteStreams.toByteArray(in);
                if (bytes.length == 0) {
                    throw new RuntimeException("无可下载文件");
                }
                return bytes;
            }
        } catch (Exception e) {
     ...
     * @param savePath   保存目录
     */
    public static void downloadFile(URLConnection connection, Path savePath) {
        try {
